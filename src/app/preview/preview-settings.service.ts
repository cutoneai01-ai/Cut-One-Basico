...
/**
 * Doble de `SettingsService` para `/__preview` (RF-TT03 §5): mismo shape público que `LandingPage`
 ...
 *
 * **A propósito no inyecta `HttpClient`.** Es la garantía "cero peticiones por construcción" de la que
 * habla el RF: si esta clase no tiene el símbolo `http` en ningún lado, no hay una sola línea capaz de
 ...
 *
 * `implements Pick<SettingsService, …>`: el RF advierte que `useClass`/`useExisting` no comprueban la
...
export class PreviewSettingsService implements Pick<SettingsService, 'branding' | 'loading' | 'ensureLoaded'> {
  ...
  /**
   * `theme.branding` es opcional en `cob-preview:theme` (RF-TT03 §7): GestionCutOne puede mandar el
   * nombre y el logo reales del tenant para que el operador evalúe el tema sobre la marca real en vez
   * de sobre "Barbería Ejemplo". Es **solo lectura** — mostrar no es editar (decisión 2 de la serie) —
