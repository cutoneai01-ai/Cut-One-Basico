// RF-TT03 §4 y §6.2. Funciones puras a propósito — nada de `document`/`window` leído por dentro—, para
// poder probar la parte que de verdad protege sin tener que simular un iframe de otro origen en el
// entorno de test.

/**
 * Capa 1 de §4, y la única de las tres que cubre el caso no obvio del RF: `/__preview` resuelve
 ...
 *
 * **Positiva, no negativa** (§4): en vez de enumerar orígenes que no valen (lista infinita), exige
...
export function isTrustedPreviewAncestor(options: {
  ...
  try {
    ...
    // válido o lo deja vacío), pero un `URL` que no parsea es exactamente el tipo de entrada que RN-05
...
/**
 * §6.2: validación de origen EN LOS DOS EXTREMOS. Esta es la mitad que corre en el listener de
 * `message` de la landing.
 *
 * **`!==` sobre la cadena completa, nunca `startsWith` ni `includes`** — el RF cita el ejemplo
